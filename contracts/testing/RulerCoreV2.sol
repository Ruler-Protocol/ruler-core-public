 ...
 */
contract RulerCoreV2 is Ownable, IRulerCore, IERC3156FlashLender, ReentrancyGuard {
  ...
  /// @notice collateral => minimum collateralization ratio
  ...
  /// @notice market make deposit, deposit paired Token to received rcTokens
  function mmDeposit(
    address _col,
    address _paired,
    uint48 _expiry,
  ...
  function redeem(
    ...
    _sendAndAccuFees(IERC20(_col), colAmountToPay, pair.feeRate, true);
  ...
  function collect(
    ...
    IERC20 pairedToken = IERC20(_paired);
    uint256 rrTokenAmtDefaulted = pair.rrToken.totalSupply();
    if (rrTokenAmtDefaulted == 0) { // no default, send paired Token to sender
      // no fees accrued as it is accrued on Borrower payment
      _sendAndAccuFees(pairedToken, _rcTokenAmt, pair.feeRate, false);
    } else {
      // total amount of rrTokens ever minted (converted from total collateral received, redeemed collateral not counted)
      uint256 rrTokenEverMinted = _getRTokenAmtFromColAmt(pair.colTotal, _col, _paired, pair.mintRatio);
      // paired token amount to pay = rcToken amount * (1 - default ratio)
      uint256 pairedTokenAmtToCollect = _rcTokenAmt * (rrTokenEverMinted - rrTokenAmtDefaulted) / rrTokenEverMinted;
      // no fees accrued as it is accrued on Borrower payment
      _sendAndAccuFees(pairedToken, pairedTokenAmtToCollect, pair.feeRate, false);
      ...
      uint256 colAmountToCollect = colAmount * rrTokenAmtDefaulted / rrTokenEverMinted;
      // accrue fees on defaulted collateral since it was never accrued
      _sendAndAccuFees(IERC20(_col), colAmountToCollect, pair.feeRate, true);
    }
  ...
  function updateCollateral(address _col, uint256 _minColRatio) external override onlyOwner {
    require(_minColRatio >= 0.5 ether, "Ruler: min colRatio < 50%");
  ...
  function viewCollectible(
    ...
    uint256 rrTokenAmtDefaulted = pair.rrToken.totalSupply();
    if (rrTokenAmtDefaulted == 0) { // no default, transfer paired Token
      pairedAmtToCollect =  _rcTokenAmt;
    } else {
      // total amount of rrTokens ever minted (converted from total collateral received, redeemed collateral not counted)
      uint256 rrTokenEverMinted = _getRTokenAmtFromColAmt(pair.colTotal, _col, _paired, pair.mintRatio);
      // paired token amount to pay = rcToken amount * (1 - default ratio)
      pairedAmtToCollect = _rcTokenAmt * (rrTokenEverMinted - rrTokenAmtDefaulted) / rrTokenEverMinted;

      // default collateral amount to pay = converted collateral amount (from rcTokenAmt) * default ratio
      uint256 colAmount = _getColAmtFromRTokenAmt(_rcTokenAmt, _col, address(pair.rcToken), pair.mintRatio);
      colAmtToCollect = colAmount * rrTokenAmtDefaulted / rrTokenEverMinted;
    }
  }
  ...
  function _sendAndAccuFees(IERC20 _token, uint256 _amount, uint256 _feeRate, bool _charge) private {
